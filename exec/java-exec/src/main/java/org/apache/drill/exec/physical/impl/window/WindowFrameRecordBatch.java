...
public class WindowFrameRecordBatch extends AbstractSingleRecordBatch<WindowPOP> {
  ...
  private void getIndex(ClassGenerator<WindowFramer> g) {
  ...
  private WindowFramer createFramer() throws SchemaChangeException, IOException, ClassTransformationException {
    ...
    for (VectorWrapper wrapper : incoming) {
      ...
      if(!popConfig.isLastWindow() || wrapper.getField().getPath().getRootSegment().getPath().matches("^w\\\\i\\$p\\\\i")) {
    ...
    getIndex(cg);
    WindowFramer agg = context.getImplementationClass(cg);
    agg.setup(
        context,
        incoming,
        this,
        WindowFrameTemplate.UNBOUNDED, WindowFrameTemplate.CURRENT_ROW
    );
    return agg;
  }
  ...
  private void setupIsSameFromBatch(ClassGenerator<WindowFramer> cg, LogicalExpression[] keyExprs) {
  ...
  private void setupIsSame(ClassGenerator<WindowFramer> cg, LogicalExpression[] keyExprs) {
  ...
  private void addRecordValues(ClassGenerator<WindowFramer> cg, LogicalExpression[] valueExprs) {
  ...
  protected IterOutcome doWork() {
    WindowFramer.AggOutcome out = framer.doWork();

    while (out == WindowFramer.AggOutcome.UPDATE_AGGREGATOR) {
      framer = null;
      try {
        setupNewSchema();
    ...
    if (out == WindowFramer.AggOutcome.RETURN_AND_COMPLETE) {
      done = true;
    }
